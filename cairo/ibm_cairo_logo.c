...
travel_path (cairo_t *cr)
{
   ...
   cairo_set_source_rgba (cr, .39, .42, .69, 1);
   ...
   */
   ...
   cairo_set_source_rgba (cr, .9, .42, .69, 1);
...
static void draw_gtk (GtkWidget      *widget,
          GdkEventExpose *eev,
          gpointer        data)
{

  cairo_t *cr;
//  cr = gdk_cairo_create (widget->window)  gtk_widget_get_window;
//   cr= cairo_create(surface);  
 // cr = gdk_cairo_create (gtk_widget_get_window());//  gtk_widget_get_window;
  travel_path (cr);
  cairo_destroy (cr);
}
      ...
      char **argv)
{
    ...
    /* Postscript Backend */
    surface = cairo_ps_surface_create ("IBM.ps", WIDTH, HEIGHT);
    draw (surface);
    cairo_surface_destroy (surface);
...
//   g_signal_connect (G_OBJECT (canvas), "expose-event", G_CALLBACK (draw_gtk), NULL);
